...
func apply_upgrade(upgrade_type: String):
	match upgrade_type:
		...
		"health":
			player_stats.max_health_bonus += 1
		"magnet":
			player_stats.magnet_range_bonus += 15
		"weapon":
			player_stats.weapon_speed_multiplier += 0.15
...
func get_stat(stat_name: String):
	if player_stats.has(stat_name):
		...
		match stat_name:
			"fire_rate_multiplier", "speed_multiplier", "weapon_speed_multiplier", "damage_multiplier":
				return 1.0
